...
///
/// For each section:
/// - If it fits entirely, include it.
/// - If it partially fits, truncate to fill the remaining budget and mark `[TRUNCATED_TOTAL_CAP]`.
/// - If no budget remains, exclude it.
pub fn apply_total_cap(sections: &mut Vec<Section>, total_max_chars: usize) {
    let mut accumulated: usize = 0;

    for section in sections.iter_mut() {
        if !section.included {
            // already excluded by caller (e.g. BOOTSTRAP.md on non-first-run)
            continue;
        }
        ...
        if accumulated + section_len <= total_max_chars {
            ...
            // Partial fit — truncate to fill remaining budget
            ...
            // No budget left — exclude this file
...
mod tests {
    ...
    fn total_cap_excludes_overflow() {
        ...
        // B partially fits (4 chars remaining of its 6)
